...
double** ajusterTaille(double** M, int n, int nv_n){
    int i,j;

    double** nvM=initialiserMatrice(nv_n);
    for (i = 0; i < n; i++) {
        for ( j = 0; j < n; j++) {
            nvM[i][j] = M[i][j];
        }
    }
    // On initialise nvM=M de nouvelle taille n 2^k et on mets des 1 sur la diagonale 
    for (i=n;i<nv_n;i++) {
        nvM[i][i] = 1;
    }
    return nvM;
...
int main(){
    ...
    nv_n=n;
    while(estPuissanceDeDeux(nv_n)==0){
        nv_n++;
    }
    double** nvA=ajusterTaille(A,n,nv_n);
    libererMatrice(n,A);
    // -------------------------------------------------- Esapce de Test -------------------------------------------------

    double** AtAinv=inverserMatrices(nv_n,multiplicationStrassen(transposerMatrices(nv_n,nvA),nvA,nv_n));
    double** Ainv=multiplicationStrassen(AtAinv,transposerMatrices(nv_n,nvA),nv_n);
